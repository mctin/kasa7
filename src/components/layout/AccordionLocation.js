...
import ChevronLocation from "../layout/ChrevronLocation";

// STYLE
import "../layout/Accordionlocation.css";
...
export default function AccordionLocation(props) {
  ...
  function toggleAccordionLocation() {
    ...
    setRotateState(
      setActive === "active"
        ...
        : "accordionlocation__icon rotate"
    );
  }
  ...
  return (
    <div className="accordionlocation__section">
      <button
        className={`accordionlocation ${setActive}`}
        onClick={toggleAccordionLocation}
      >
      ...
      >
        <div>
          <div className="accordionlocation__text">{props.content}</div>
        </div>
      </div>
    </div>
  );
